...
def sort_leads(row):
    try:
        if pd.isnull(row['Time1']) or pd.isnull(row['Time2']):
            return row  # Don't sort if either time is missing
        t1 = pd.to_datetime(row['Time1'], errors='coerce')
        t2 = pd.to_datetime(row['Time2'], errors='coerce')
        if pd.notnull(t1) and pd.notnull(t2) and t2 < t1:
