...
# Ensure earlier appointment is on the left
def sort_leads(row):
            try:
            except Exception as e:
                print("Drive time error:", e)
                continue
                    ...
                    print('Error in try block:', e)
        if pd.to_datetime(row['Time2']) < pd.to_datetime(row['Time1']):
                for suffix in ['', '1', '2']:
                row[f'Lead{suffix}'], row[f'Customer{suffix}'], row[f'Time{suffix}'], row[f'City{suffix}'] = (
                    row.get(f'Lead2'), row.get(f'Customer2'), row.get(f'Time2'), row.get(f'City2')
                ) if suffix == '' else (
                    row.get(f'Lead1'), row.get(f'Customer1'), row.get(f'Time1'), row.get(f'City1')
                ) if suffix == '2' else (
                    row.get(f'Lead2'), row.get(f'Customer2'), row.get(f'Time2'), row.get(f'City2')
                )
    except:
...
df_result = df_result.apply(sort_leads, axis=1)
