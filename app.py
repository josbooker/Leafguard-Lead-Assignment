...
# Prepare route lines between paired leads
route_lines = []
for _, row in df_result.iterrows():
    if row['Type'] == 'paired':
        if pd.notnull(row['lat1']) and pd.notnull(row['lon1']) and pd.notnull(row['lat2']) and pd.notnull(row['lon2']):
            route_lines.append({
                'start': [row['lon1'], row['lat1']],
                'end': [row['lon2'], row['lat2']]
            })
