...
if uploaded_file:
    ...
    # FIRST: Assign all short-distance pairs
    for i, j, mins in sorted(short_pairs, key=lambda x: x[2]):
        if i not in used and j not in used and len(used) + 2 <= max_capacity:
    ...
    # THEN: If we haven't hit rep capacity, assign long pairs only if needed
    if len(used) < max_capacity:
    ...
    for row in df_result.itertuples():
        ...
        if pd.notna(row.lat1) and pd.notna(row.lon1):
            map_data.append({"position": [row.lat1, row.lon1], "tooltip": f"Rep: {rep_id}, Lead: {row.Lead1}", "color": color})
        if pd.notna(row.lat2) and pd.notna(row.lon2):
            map_data.append({"position": [row.lat2, row.lon2], "tooltip": f"Rep: {rep_id}, Lead: {row.Lead2}", "color": color})

    if map_data:
        layer = pdk.Layer("ScatterplotLayer", data=map_data, get_position="position", get_fill_color="color", get_radius=150, pickable=True)
        view_state = pdk.ViewState(latitude=map_data[0]["position"][0], longitude=map_data[0]["position"][1], zoom=7)
...
if uploaded_file:
    ...
    # Assign all short-distance pairs first
    for i, j, mins in sorted(short_pairs, key=lambda x: x[2]):
        if i not in used and j not in used and len(used) + 2 <= max_capacity:
            assignments.append((rep, i, j, mins))
            used.update([i, j])
    ...
    # Calculate remaining rep slots before considering long-distance pairs
    ...
    for i, j, mins in sorted(long_pairs, key=lambda x: x[2]):
        if i not in used and j not in used and long_pairs_needed > 0:
            assignments.append((rep, i, j, mins))
            used.update([i, j])
            rep += 1
            long_pairs_needed -= 2
    ...
    if map_df:
        ...
        layer = pdk.Layer("ScatterplotLayer", data=map_df, get_position=["lon", "lat"], get_fill_color="color", get_radius=150, pickable=True)
        view_state = pdk.ViewState(latitude=map_df["lat"].mean(), longitude=map_df["lon"].mean(), zoom=7)
        st.pydeck_chart(pdk.Deck(layers=[layer], initial_view_state=view_state, tooltip={"text": "{tooltip}"}))
