...
if uploaded_file:
    ...
    for i, j, mins in df_dur.itertuples(index=False):
        ...
        if mins <= max_drive:
            valid_pairs.append((i, j, mins, delta_hours))

    assignments = []
    used = set()
    rep = 1
    for i, j, mins, _ in sorted(valid_pairs, key=lambda x: x[2]):
    ...
    total_needed = len(assignments) + len(single_leads)
    can_assign = rep_count * 2
    extras = total_needed - can_assign
    reschedules = set()

    if extras > 0:
        reschedules = set(single_leads[-extras:])
        single_leads = single_leads[:-extras]
    ...
    color_palette = [[255,0,0],[0,128,0],[0,0,255],[255,165,0],[128,0,128],[0,206,209],[255,20,147]]
    rep_colors = {}
    map_data = []
    ...
    view_state = pdk.ViewState(latitude=df_result["lat1"].mean(), longitude=df_result["lon1"].mean(), zoom=7)
