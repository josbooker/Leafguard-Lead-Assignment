...
if uploaded_file:
    ...
    for i in range(len(df)):
        ...
        for j in range(i + 1, len(df)):
            if j in assigned:
                continue
            time2 = df.iloc[j]['Estimate Time'].time()
            addr2 = df.iloc[j]['Full Address']
            name2 = df.iloc[j]['Customer Name']
            dur = durations[i][j]
            if dur is not None and dur <= 60 and time1 != time2:
                if used_reps < num_reps_available:
                    df_result = pd.concat([df_result, pd.DataFrame([{
                        ...
                        'Time2': time2,
                        'Address2': addr2,
                        'Drive Time (min)': round(dur, 1),
                        'Rep': rep_counter,
                        'Type': 'Paired',
                        'Customer Name 1': name1,
                        'Customer Name 2': name2
                    }])])
                    assigned.update([i, j])
                    used_reps += 1
                    rep_counter += 1
                    route_lines.append({'start': [df.iloc[i]['lon'], df.iloc[i]['lat']], 'end': [df.iloc[j]['lon'], df.iloc[j]['lat']]})
                    paired = True
                    break

        if not paired and i not in assigned:
            if used_reps < num_reps_available:
                df_result = pd.concat([df_result, pd.DataFrame([{
                    'Time1': time1,
                    'Address1': addr1,
