...
if uploaded_file:
    ...
    rep_colors_rgb = {str(k): f"rgb({','.join(map(str, v))})" for k, v in rep_colors.items()}

    def style_rep_column(val):
        color = rep_colors_rgb.get(str(val), "black")
        return f'color: {color}'

    styled_df = df_display.style.applymap(style_rep_column, subset=["Rep"])
...
st.success("✅ Assignments complete!")
    ...
    st.dataframe(df_display.style.applymap(style_rep_column, subset=["Rep"]))
