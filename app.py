...
# Color palette and rep color mapping
color_palette = [[255,0,0],[0,128,0],[0,0,255],[255,165,0],[128,0,128],
                 [0,206,209],[255,20,147],[50,205,50],[70,130,180],[210,105,30]]
rep_colors = {}  # maps rep id -> RGB list
map_df = []
for row in df_result.itertuples():
    ...
    rep_id = row.Rep
    color = rep_colors.get(rep_id, [50, 50, 200]) if isinstance(rep_colors, dict) else [50, 50, 200]
...
if not map_df.empty:
    ...
    # Build route lines between paired leads
    route_lines = []
    for row in df_result.itertuples():
        if row.Type == 'Paired' and pd.notna(row.lat1) and pd.notna(row.lon1) and pd.notna(row.lat2) and pd.notna(row.lon2):
            route_lines.append({'start': [row.lon1, row.lat1], 'end': [row.lon2, row.lat2]})
    ...
    view_state = pdk.ViewState(latitude=avg_lat, longitude=avg_lon, zoom=7)
