...
# ============================================
# SMART PAIRING SECTION — DRIVE TIME AWARE LOGIC
# ============================================
assigned = set()
rep_id = 1
reps_used = 0
...
for i in range(len(leads_df)):
    if i in assigned:
        continue
    lead1 = leads_df.iloc[i]
    best_pair = None
    best_drive = None

    for j in range(i+1, len(leads_df)):
        if j in assigned:
            continue
        lead2 = leads_df.iloc[j]
        ...
        # Check if the pair meets time-distance constraints
        if time_diff < 3:
            if drive > 60:
                continue
        else:  # >= 3 hours apart
            if drive > 60 and reps_used < num_reps:
                continue
            if drive > 120:
                continue
    ...
    # Assign pair if available and we still have reps
    if best_pair is not None and reps_used < num_reps:
        j = best_pair
        lead2 = leads_df.iloc[j]
        results.append({
            "Rep": rep_id,
        ...
        # No reps left — suggest to reschedule
        results.append({
            "Rep": "Reschedule",
            "Lead1": lead1["Lead/Invoice #"],
            "Customer1": lead1["Customer Name"],
